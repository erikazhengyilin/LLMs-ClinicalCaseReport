...
parser.add_argument('--model', type=str, choices=["starmpcc/Asclepius-Mistral-7B-v0.3"], required=True)
...
access_token = read_access_token('api_keys/huggingface.txt')
...
def pred_fewshot(fewshot_prompt, case_description, model, tokenizer, device):
    prompt = f"""You are a highly knowledgeable medical expert with expertise in diagnosing a \
    ...
    output_ids = model.generate(input_ids, max_new_tokens=4096)
    output = tokenizer.decode(output_ids[0][input_token_length:])
    diagnosis = output.replace('\n', '').strip()
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model, token=access_token, device_map="auto", return_tensors="pt", use_fast=False)
    model = AutoModelForCausalLM.from_pretrained(args.model, token=access_token, device_map="auto")
    device = model.device
    ...
    for i, case in tqdm(enumerate(dataset)):
        ...
        if args.sample_mode == 'random_k':
            ...
            pred = pred_fewshot(fewshot_prompt, descriptions, model, tokenizer, device)
        ...
        if not matching_row_index.empty:
            results.loc[matching_row_index, f"{args.y_attr}_{args.model}"] = pred
        else:
