...
parser.add_argument('--model', type=str, choices=["llama2", "llama3", "mistral:latest", "taozhiyuai/openbiollm-llama-3:8b_q8_0"], required=True)
...
def pred_fewshot(fewshot_prompt, case_description):
    ...
    response = ollama.chat(model=args.model,
        messages=[
    ...
    diagnosis = response['message']['content'].replace('\n', '').strip()
    return diagnosis
 
def pred_zeroshot(case_description):
...
def main():
    ...
    for i, case in tqdm(enumerate(dataset)):
        ...
        if args.sample_mode == 'random_k':
            ...
            pred = pred_fewshot(fewshot_prompt, descriptions)
            ...
            pred = pred_zeroshot(descriptions)
